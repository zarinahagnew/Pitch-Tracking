...
subj_MP_allsubs_HC=subj_MPs_allsubs(17:end);
...
for isubj = 1:nHCs
    ...
    load (sprintf('%sALLDATA.mat',patient_info{isubj}.exprdir));            % from WriteDataout.m
    ...
    for each_block=1:num_blocks;
        ...
        %                     data_1.goodstd.prestep(d)=nanstd(shiftedpitch_data{1,each_block}{1, d}(T1_pat:T2_HC));
        %                     % amp
...
%% write data out into one structure called data_1
        for each_block=1:num_blocks;
            ...
            for d=1:trialsperblock
                ...
                %pre step
                data_1.goodpitchdata(each_block).prestep_data(d,:)=shiftedpitch_data{1,each_block}(d,T1_HC:T2_HC);
                data_1.goodstd.prestep(d)=nanstd(shiftedpitch_data{1,each_block}(d,T1_HC:T2_HC));
                % amp
                data_1.goodampdata(each_block).prestep_amp(d,:)=amp_data{each_block}(d,T1_HC:T2_HC);
                data_1.goodstd.prestep_amp(d)=nanstd(amp_data{each_block}(d,T1_HC:T2_HC));
                %poststep
                data_1.goodstd.poststep(d)=nanstd(shiftedpitch_data{1,each_block}(d,T3_HC:T4_HC));
                data_1.goodpitchdata(each_block).poststep_data(d,:)=shiftedpitch_data{1,each_block}(d,T3_HC:T4_HC);
                
                data_1.goodampdata(each_block).poststep_amp(d,:)=amp_data{each_block}(d,T3_HC:T4_HC);
                data_1.goodstd.poststep_amp(d)=nanstd(amp_data{each_block}(d,T3_HC:T4_HC));
...
% cents is calculated by trial
                ...
                for iframe=1:201
                    if isnan(data_4.prestep(each_block).data(d,iframe))==1;
                        ...
                        data_5.prestep_normbytrial(each_block).data(d,iframe)=1200*log2(data_4.prestep(each_block).data(d,iframe)/nanmean(data_4.goodpitchdata(each_block).data(d,T1_HC:T2_HC)));
                ...
                for iframe=1:201
                    if isnan(data_4.poststep(each_block).data(d,iframe))==1
                        ...
                        data_5.poststep_normbytrial(each_block).data(d,iframe)=1200*log2(data_4.poststep(each_block).data(d,iframe)/nanmean(data_4.goodpitchdata(each_block).data(d,T1_HC:T2_HC)));
