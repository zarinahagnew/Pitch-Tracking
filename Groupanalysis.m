...
%% calc group distance from target
...
for condnn=1:10
subplot(2,5,condnn)
plot(nanmean(GroupData(1).cond(condnn).distfromtarg_WT), 'm')
end
...
nanmean(GroupData(1).cond(1).distfromtarg_WT)

figure
for condnn=1:10
    subplot(2,5,condnn)
    plot(nanmean(GroupData(1).cond(condnn).distfromtarg_WT_PATS), 'm','LineWidth',1.3);
    hold
    plot(nanmean(GroupData(1).cond(condnn).distfromtarg_WT_HCs), 'k','LineWidth',1.3);
    goodplot
end
