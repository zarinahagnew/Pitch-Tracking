...
% B_goodtrials_allsubs and organises them by condition. Saves this as
% 'sorted_data.mat'
...
%for each_subject = 23
    ...
    the_expr_dir_2 = [cerebellar_data_rootdir (patient_info_2{each_subject}.exprdir)];
    cd (the_expr_dir_2)

    load (sprintf('%sgooddata_spectralanalysis.mat',patient_info_2{each_subject}.exprdir));
    % in main run folder
    
    %load (sprintf('%sgooddata.mat',the_expr_dir))           % pitchdata from B_goodtrials_allsubs
    load (sprintf('%sexpr_config.mat',the_expr_dir_2))        % condition orders from fusp
    load (sprintf('%sshifted_blockalt.mat',the_expr_dir_2))   % pert resps (original, unshifted, despite the name)
...
%     end
    ...
    for each_block=1:8
        ...
        for trial=1:10
            sorted_data(blockx(trial)).wholetrial(line_counter(blockx(trial)),:)=gooddata_spec(1).goodpitchdata(each_block).data(trial,:);
            sorted_data(blockx(trial)).prestep(line_counter(blockx(trial)),:)=gooddata_spec(1).prestep(each_block).data(trial,:);
            sorted_data(blockx(trial)).poststep(line_counter(blockx(trial)),:)=gooddata_spec(1).poststep(each_block).data(trial,:);            
            sorted_data(blockx(trial)).AMP_wholetrial(line_counter(blockx(trial)),:)=gooddata_spec(1).goodampdata(1).wholetrial_amp(trial,:);                                                                                     
            sorted_data(blockx(trial)).AMP_prestep(line_counter(blockx(trial)),:)=gooddata_spec(1).goodampdata(1).prestep_amp(trial,:);
            sorted_data(blockx(trial)).AMP_poststep(line_counter(blockx(trial)),:)=gooddata_spec(1).goodampdata(1).poststep_amp(trial,:);
            sorted_data(blockx(trial)).distfromtarg_pre=gooddata_spec(1).pre_distfromtarget(1).data(trial);
            sorted_data(blockx(trial)).distfromtarg_post=gooddata_spec(1).post_distfromtarget(1).data(trial);
            sorted_data(blockx(trial)).pertresp(line_counter(blockx(trial)),:)=shifted_blockalt{trial, each_block};
            sorted_data(blockx(trial)).goodstdwholetrial(line_counter(blockx(trial)),:)=gooddata_spec(1).goodstd(1).wholetrial;
            sorted_data(blockx(trial)).goodstdprestep(line_counter(blockx(trial)),:)=gooddata_spec(1).goodstd(1).prestep;
            sorted_data(blockx(trial)).goodstdpoststep(line_counter(blockx(trial)),:)=gooddata_spec(1).goodstd(1).poststep;
            sorted_data(blockx(trial)).mean_goodstdwholetrial(line_counter(blockx(trial)),:)=nanmean(gooddata_spec(1).goodstd(1).wholetrial);
            sorted_data(blockx(trial)).mean_goodstdprestep(line_counter(blockx(trial)),:)=nanmean(gooddata_spec(1).goodstd(1).prestep);
            sorted_data(blockx(trial)).mean_goodstdpoststep(line_counter(blockx(trial)),:)=nanmean(gooddata_spec(1).goodstd(1).poststep);
            sorted_data(blockx(trial)).goodstdwholetrial_amp(line_counter(blockx(trial)),:)=gooddata_spec(1).goodstd(1).wholetrial_amp;
            sorted_data(blockx(trial)).goodstdprestep_amp(line_counter(blockx(trial)),:)=gooddata_spec(1).goodstd(1).prestep_amp;
            sorted_data(blockx(trial)).goodstdpoststep_amp(line_counter(blockx(trial)),:)=gooddata_spec(1).goodstd(1).poststep_amp;
            
            sorted_data(blockx(trial)).mean_goodstdwholetrial_amp(line_counter(blockx(trial)),:)=nanmean(gooddata_spec(1).goodstd(1).wholetrial_amp);
            sorted_data(blockx(trial)).mean_goodstdprestep_amp(line_counter(blockx(trial)),:)=nanmean(gooddata_spec(1).goodstd(1).prestep_amp);
            sorted_data(blockx(trial)).mean_goodstdpoststep_amp(line_counter(blockx(trial)),:)=nanmean(gooddata_spec(1).goodstd(1).poststep_amp);

            %sorted_data(blockx(trial)).audiodata(line_counter(blockx(trial)),:)=gooddata_spec.audio(each_block).audio(trial,:);
    ...
    save sorted_data_spectral sorted_data
