...
stdev_cutoff_pre = 10.36;
stdev_cutoff_post =17.13;
stdev_cutoff_wholetrial = 29.40;

% define the stdevs to exlude for the ampl data:
stdev_cutoff_pre_amp = 3.91;
stdev_cutoff_post_amp = 5.97;
stdev_cutoff_wholetrial_amp = 8.98;
...
%% REMOVE BAD TRIALS ACCORDING TO STDEV:
            ...
            for d=1:voiceonset.ntrials % 10
                if nanstd(pitch_data{1}{d})<stdev_cutoff_wholetrial
                    ...
                    goodpertresp(each_block).data{d}=shifted_blockalt{d,each_block};
                ...
                for t=1:1:voiceonset.ntrials %trial loop
                    for s=1:nframes_to_use %data point loop
                        if goodampdata(1,each_block).wholetrial_amp(t,s)>amp_thresh
                            gooddata.wholetrial(each_block).wholetrial(t,s)=goodpitchdata(each_block).data(t,s);
                    ...
                    gooddata.wholetrial_std(t)=nanstd(gooddata.wholetrial(each_block).wholetrial(t,:));
                end
                ...
                for t=1:1:voiceonset.ntrials %trial loop
                    for s=1:201 %data point loop
                        if goodampdata(1,each_block).prestep_amp(t,s)>amp_thresh;
                            gooddata.prestep(each_block).data(t,s)=goodpitchdata(each_block).prestep_data(t,s); %% working from here
                ...
                for t=1:1:voiceonset.ntrials %trial loop
                    for s=1:201 %data point loop
                        if goodampdata(1,each_block).poststep_amp(t,s)>amp_thresh;
                            ...
                            %gooddata.poststep(1,each_block).data(t,:)
...
for each_block=1:8
for itrial=1:10
    plot(frame_taxis(1:frame_taxis_to_use),pitch_data{each_block}{itrial}(1, 1:1370))
    title(sprintf('trial(%d)',t));
    hold on
