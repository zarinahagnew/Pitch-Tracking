    ...
    for each_block=1:num_blocks;
        ...
        for d=1:voiceonset.ntrials
            if nanstd(shiftedpitch_data{1,each_block}{1, d}(T1:T2))< stdev_cutoff_pre
                ...
                goodpitchdata(each_block).prestep_data(d,:)=temp1.goodpitchdata(each_block).prestep_data(d,:);
                goodstd.prestep(d)=temp1.goodstd.prestep(d);
            else
        ...
        for d=1:voiceonset.ntrials
            if nanstd(amp.data(d,T1:T2))< stdev_cutoff_pre_amp
                ...
                goodampdata(each_block).prestep_amp(d,:)=temp1.goodampdata(each_block).prestep_amp(d,:);
                goodstd.prestep_amp(d)=temp1.goodstd.prestep_amp(d);
            else
        ...
        for d=1:voiceonset.ntrials
            if nanstd(shiftedpitch_data{1,each_block}{1, d}(T3:T4))< stdev_cutoff_post
                ...
                goodstd.poststep(d)=temp1.goodstd.poststep(d);
                goodpitchdata(each_block).poststep_data(d,:)=temp1.goodpitchdata(each_block).poststep_data(d,:)
            else
        ...
        for d=1:voiceonset.ntrials % 10
            if nanstd(amp.data(d,T3:T4))< stdev_cutoff_post_amp
                ...
                goodampdata(each_block).poststep_amp(d,:)=temp1.goodampdata(each_block).poststep_amp(d,:);
                goodstd.poststep_amp(d)=temp1.goodstd.poststep_amp(d);
            else
        ...
        % window data pre step
        for t=1:voiceonset.ntrials                                        %trial loop
            for s=1:length(goodampdata(1,each_block).prestep_amp(1,:));     % data point loop (201 long)
...
% for a=1:10;
%     subplot(2,2,1)
%     plot(frame_taxis_to_use,goodpitchdata(each_block).data(a,:))
%     axis([0 4 0 300])
%     subplot(2,2,2)
%     plot(frame_taxis_to_use,goodampdata(each_block).wholetrial_amp(a,:), 'r')
%     axis([0 4 0 500000000])
%
%     subplot(2,2,3)
%     plot(frame_taxis_to_use,gooddata.wholetrial(each_block).wholetrial(a,:))
%         axis([0 4 0 300])
%
%     subplot(2,2,4)
%     plot(frame_taxis_to_use,gooddata.AMP_wholetrial(each_block).data(a,:), 'r')
%     axis([0 4 0 500000000])
%     pause;
% end
